...
# Does not work like in the example provided since there is always a space needed between the number and string.
# And what would happen by 100 iterations?? Best is to make space between the number and the string
# produce the following right-aligned output (DO NOT use a list):

number = 2
for i in range(0, 11):
    result = number ** i
    print(f"{number} ^ {i:<2} is {result:>5}")
# 2 ^ 0 is    1
# 2 ^ 1 is    2
# 2 ^ 2 is    4
# 2 ^ 3 is    8
